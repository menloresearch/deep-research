...
# Setup OpenTelemetry instrumentation for comprehensive agent tracing
def setup_telemetry():
    """Setup OpenTelemetry instrumentation for smolagents"""
    try:
        # Try Phoenix setup first (simpler, local)
        try:
            ...
            logger.info("OpenTelemetry instrumentation setup with Phoenix - navigate to http://0.0.0.0:6006/projects/ to view traces")
            logger.info("To start Phoenix server: python -m phoenix.server.main serve")
            return "phoenix"
            
        except ImportError:
            logger.info("Phoenix not available, trying Langfuse setup...")
            
            # Try Langfuse setup as fallback
            try:
                ...
                if os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT") and os.getenv("OTEL_EXPORTER_OTLP_HEADERS"):
                    trace_provider = TracerProvider()
                    trace_provider.add_span_processor(SimpleSpanProcessor(OTLPSpanExporter()))
                    SmolagentsInstrumentor().instrument(tracer_provider=trace_provider)
                    logger.info("OpenTelemetry instrumentation setup with Langfuse")
                    return "langfuse"
                else:
                    logger.warning("Langfuse environment variables not set (OTEL_EXPORTER_OTLP_ENDPOINT, OTEL_EXPORTER_OTLP_HEADERS)")
                    
            except ImportError:
                logger.warning("Langfuse dependencies not available")
                
        # Fallback to basic instrumentation
        try:
            from openinference.instrumentation.smolagents import SmolagentsInstrumentor
            SmolagentsInstrumentor().instrument()
            logger.info("Basic OpenTelemetry instrumentation setup (no backend)")
            return "basic"
        except ImportError:
            logger.warning("OpenTelemetry instrumentation not available - install with: pip install 'smolagents[telemetry]'")
            return None
...
telemetry_backend = setup_telemetry()
logger.info(f"Telemetry backend: {telemetry_backend}")
...
try:
    search_result = web_search(query="Donald Trump news")
    logger.info(f"Web search test successful. Result length: {len(str(search_result))}")
    # Log the actual result for debugging
    logger.debug(f"Web search result: {str(search_result)[:1000]}...")
    print(search_result)  # This will now be captured in logs too
except Exception as e:
    logger.error(f"Web search test failed: {str(e)}")
    logger.exception("Web search exception details:")
    print(f"Web search failed: {e}")

# Log all imported tools and their status
logger.info("All custom tools imported and ready")
...
logger.info("Loading environment variables and logging into HuggingFace...")
load_dotenv(override=True)
login(os.getenv("HF_TOKEN"))
logger.info("HuggingFace login completed")
...
def create_agent():
    ...
    browser = SimpleTextBrowser(**BROWSER_CONFIG)
    logger.debug("Browser instance created for agent")
    ...
    logger.debug(f"Created {len(web_tools_for_agent)} web tools for agent")

    agent = CodeAgent(
        model=model,
        tools=[visualizer] + web_tools_for_agent,
        max_steps=10,
        verbosity_level=2,  # Maximum verbosity to capture all internal logs
        additional_authorized_imports=AUTHORIZED_IMPORTS,
        planning_interval=4,
    )
    
    creation_time = time.time() - start_time
    logger.info(f"Agent created successfully in {creation_time:.2f} seconds. Total tools: {len(agent.tools)}")
    return agent
...
class GradioUI:
    ...
    def interact_with_agent(self, prompt, messages, session_state):
        ...
        try:
            ...
            logger.info(f"Agent interaction completed. Steps: {step_count}, Processing time: {processing_time:.2f}s, Total time: {total_time:.2f}s")
            yield messages
    ...
    def process_csv_file(self, input_csv_file_obj, progress=gr.Progress(track_tqdm=True)):
        ...
        try:
            ...
            def process_row(row_data_with_index):
                ...
                agent = create_agent()  # Create a new agent for each row
                ...
                try:
                    ...
                    logger.info(f"Row {idx+1} processed successfully in {processing_time:.2f}s. Messages: {message_count}")
            ...
            logger.info(f"Starting sequential processing for {total_queries} queries")
            processing_start = time.time()
            
            for i in range(total_queries):
                try:
                    processed_row_data, original_idx = process_row((i, input_row_dicts[i]))
                    ...
                    failed_row_data = input_row_dicts[i].copy()
                    failed_row_data["answer"] = f"Failed to process row due to: {exc}"
                    
                    logger.error(f"Row {i+1} failed with exception: {str(exc)}")
                    logger.exception(f"Row {i+1} full exception:")
            ...
            logger.info(f"Total time: {total_time:.2f}s, Processing time: {processing_time:.2f}s")
    ...
    def launch(self, **kwargs):
        logger.info("Starting Gradio UI launch...")
        logger.debug(f"Launch arguments: {kwargs}")
        ...
        with gr.Blocks(theme="ocean", fill_height=True) as demo:
            ...
            with gr.Tabs():
                with gr.TabItem("Interactive Chat"):
                    logger.debug("Setting up Interactive Chat tab")
                ...
                with gr.TabItem("Batch Processing (CSV)"):
                    logger.debug("Setting up Batch Processing tab")
                    with gr.Column():
                        gr.Markdown(
            ...
            logger.info("Gradio interface setup completed, starting demo...")
...
if __name__ == "__main__":
    ...
    logger.info(f"Environment variables: HF_TOKEN={'SET' if os.getenv('HF_TOKEN') else 'NOT SET'}, SERPAPI_API_KEY={'SET' if os.getenv('SERPAPI_API_KEY') else 'NOT SET'}")
    
    # Log telemetry setup instructions
    logger.info("=== TELEMETRY SETUP INSTRUCTIONS ===")
    logger.info("For comprehensive agent tracing, install telemetry dependencies:")
    logger.info("Option 1 (Phoenix - Local): pip install 'smolagents[telemetry]' && pip install arize-phoenix")
    logger.info("  Then run: python -m phoenix.server.main serve")
    logger.info("  View traces at: http://0.0.0.0:6006/projects/")
    logger.info("Option 2 (Langfuse - Cloud): pip install 'smolagents[telemetry]' opentelemetry-sdk opentelemetry-exporter-otlp")
    logger.info("  Set environment variables: OTEL_EXPORTER_OTLP_ENDPOINT and OTEL_EXPORTER_OTLP_HEADERS")
    logger.info("==========================================")
