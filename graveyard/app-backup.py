...
model = LiteLLMModel(
    model_id="openrouter/qwen/qwen3-30b-a3b",
    custom_role_conversions=custom_role_conversions,
)
...
class GradioUI:
    ...
    def process_csv_file(self, input_csv_file_obj, progress=gr.Progress(track_tqdm=True)):
        ...
        output_rows = []  # Will store dictionaries, each representing a row for the output CSV
        agent = create_agent()

        try:
            df = pd.read_csv(input_file_path, keep_default_na=False)  # keep_default_na=False to preserve empty strings
            ...
            for i, row_data in enumerate(input_row_dicts):
                query_text = row_data.get("query")  # Get query from the current row

                progress(
                    (i + 1) / total_queries,
                    desc=f"Processing row {i + 1}/{total_queries}: query '{str(query_text)[:30]}...'",
                )

                current_output_row = row_data.copy()  # Start with all original fields and their values

                if pd.isna(query_text) or not isinstance(query_text, str) or not str(query_text).strip():
                    current_output_row["answer"] = "Skipped: Invalid, empty, or non-string query in 'query' column"
                ...
                try:
                    ...
                    print(f"Error detail for query '{str(query_text)}': {e}")
            ...
            print(f"Detailed error during CSV processing: {e}")  # Log detailed error
            return None

        if not output_rows:
            gr.Warning("No data to write to output CSV.")
            return None
        ...
        if "answer" not in output_fieldnames:
            ...
            # If 'answer' exists but is not last, remove and append to make it last
            output_fieldnames.remove("answer")
            output_fieldnames.append("answer")

        original_filename = os.path.basename(input_file_path)
        output_filename = f"results_{original_filename}"
        ...
        try:
            ...
            gr.Error(f"Error writing output CSV: {str(e)}")
            print(f"Detailed error during CSV writing: {e}")  # Log detailed error
            return None
        ...
        with gr.Blocks(theme="ocean", fill_height=True) as demo:
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("Batch Processing (CSV)"):
                    with gr.Column():
                        gr.Markdown(
                            "Upload a CSV file with a 'query' column. Each query will be processed, and the results will be available for download as a new CSV file."
                        )
