...
def initialize_local_search(cli_processed_data_dir_str: str, config_default_path: Path) -> bool:
    ...
    if search_module.vectorstore is None:
        print(f"Failed to load vector store from {search_module.PROCESSED_DATA_DIR}. Exiting.")
        return False
...
def perform_local_search(query: str, top_k: int = RAG_SEARCH_RESULTS_COUNT) -> list[str]:
    """Performs a local RAG search and returns top_k document contents as a list of strings."""
    if search_module.vectorstore is None:
        print("Error: Vectorstore not loaded. Ensure it's initialized before searching.")
        return []
    try:
        ...
        if isinstance(results_from_search_module, list):
            if all(isinstance(item, str) for item in results_from_search_module):
                return typing.cast(list[str], results_from_search_module)  # Cast to list[str] for clarity
            else:
        ...
        print(f"An unexpected error occurred during local search for query '{query}': {e}")
        return []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Simple CLI Research Agent with Local RAG")
    ...
    if not initialize_local_search(args.processed_data_dir, DEFAULT_PROCESSED_DATA_DIR_CONFIG):
    ...
    if not search_query:
        print("Failed to generate search query for local RAG. Using original question as query.")
    ...
    if not retrieved_documents:
        print("No documents retrieved from local RAG or an error occurred during search. Exiting.")
        return
